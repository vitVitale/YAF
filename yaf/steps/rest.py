...
class RestSteps(Base):
    ...
    def send_rest_request(client_name, payload, expected_status):
        ...
        info_mess = 'Document sent via REST - \n'
    ...
    @staticmethod
    @allure.step('Send asynchronous REST request')
    ...
    @staticmethod
    @allure.step('Get asynchronous REST response')
...
########################################################################################################################
    ...
    def attach_response_block(response, mess):
        if response.headers is not None:
            info_mess = 'Headers received from REST response - \n'
            msg = '<table border="0">'
        ...
        if response.text is not None:
            info_mess = 'Document received from REST response - \n'
    ...
    def expected_check(expected_value, actual):
        ...
        assert str(actual) == expected_value, f'Http status [{actual}] did not match ' \
                                              f'with expected [{expected_value}] !!'
